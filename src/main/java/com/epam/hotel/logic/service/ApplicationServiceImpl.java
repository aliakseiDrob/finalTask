...
public class ApplicationServiceImpl implements com.epam.hotel.logic.service.api.ApplicationService {
    ...
    public List<Application> findPageApplicationsInProgress(int itemsPerPage, int page) throws ServicesException {
        try (DaoHelper daoHelper = daoHelperFactory.create()) {
            ApplicationDao dao = daoHelper.createApplicationDao();
            int begin = (page - 1) * itemsPerPage;
            return dao.findLimitApplicationsInProgress(itemsPerPage, begin);  //TODO
        } catch (DaoException | SQLException e) {
            throw new ServicesException(e);
        }
    }
    ...
    public long findAmountPagesInProgress(int itemsPerPage) throws ServicesException {
        try (DaoHelper daoHelper = daoHelperFactory.create()) {
            ApplicationDao dao = daoHelper.createApplicationDao();
            long totalApplications = dao.findNumberRecordsInProgress();
            if (totalApplications <= itemsPerPage) {
                return 1;
            }
            return (int) Math.ceil(totalApplications / (double) itemsPerPage);
    ...
    @Override
    public long findAmountByUserId(long id, int itemsPerPage) throws ServicesException {
        try (DaoHelper daoHelper = daoHelperFactory.create()) {
            ApplicationDao dao = daoHelper.createApplicationDao();
            long totalApplications = dao.findNumberRecordsByUserId(id);
            if (totalApplications <= itemsPerPage) {
                return 1;
            }
            return (int) Math.ceil(totalApplications / (double) itemsPerPage);
